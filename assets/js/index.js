...
var questionsArray = [
  {
    question:
      "From the given array, which index is the letter 'd' on ['a','b', 'c', 'd']? ",
    choices: ["0", "3", "4", "1"],
    answer: "3",
  },
  {
    question:
      "Inside the HTML document, where do you place the JavaScript code? ",
    choices: [
      "in the <footer> element",
      "Inside the script tag ",
      "inside the title tag",
      "You don't have to add any tag for Javascript",
    ],
...
submit.addEventListener("click", function (event) {
  ...
  highScore.push({ initials: initialValue, score: userScore });
  localStorage.setItem("userData", JSON.stringify(highScore));

  console.log(highScore);

  //List is created with new high scores that were inputted
  ...
  for (var i = 0; i < highScore.length; i++) {
    ...
    li.innerHTML = `${highScore[i].initials} - ${highScore[i].score} `;
    ol.appendChild(li);
  }
});
